...
TEST_CASE("Quantization and dequantization") {
  auto x = tensor::rand<float>({2, 3, 4});
...
TEST_CASE("Quantized matrix multiplication") {
  auto x = tensor::rand<float>({3, 1, 2, 2});
  auto y = tensor::rand<float>({3, 4, 2, 5});
  ...
  REQUIRE(tensor::all(tensor::is_close(dres, dexp, 1e-3, 1e-3)).item());
}
