...
public:
    ...
    {
        assert(("All rows must be the same length!", std::adjacent_find(init.begin(), init.end(), [](auto l, auto r) { return l.size() != r.size(); }) == init.end()));
        _data.reserve(_rows * _cols);
        std::for_each(init.begin(), init.end(), [this](auto list) {
