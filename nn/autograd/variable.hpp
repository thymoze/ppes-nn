...
public:
    ...
    void add_grad(const Variable<T>& grad) const {
        if (shared_grad->grad) {
            shared_grad->grad = std::make_unique<Variable<T>>(
                Variable(shared_grad->grad->value() + grad.value())
            );
        } else {
    ...
    void zero_grad() const {
        shared_grad->grad.reset();
    }

    Variable<T>& grad() const {
...
public:
    ...
    Matrix<T>::ref operator() (size_t i, size_t j) {
        return value().operator()(i, j);
    }

    Matrix<T>::const_ref operator() (size_t i, size_t j) const {
        return value().operator()(i, j);
    }
